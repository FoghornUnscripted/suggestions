...
function checkAuth() {
    if (window.localStorage["suggestions_passwordhash"] != passwordHash) {
        $("#protected_content").html('<div class="text-center mb-3 mt-3">' +
                               '<img src="../assets/images/logo.svg" alt="Foghorn Unscripted" height="100">' +
                             '</div>' +
                             '<div class="row d-flex justify-content-center">' +
                               '<div class="col-lg-4 col-sm-12">' +
                                 '<form onsubmit="setPassword()">' +
                                   '<input type="password" class="form-control mb-1" autofocus id="password_input" />' +
                                   '<button type="submit" class="btn btn-primary btn-block">Login</button>' +
                                 '</form>' +
                               '</div>' +
                             '</div>');
        $("#protected_content").removeClass("d-none");
        return false;
    } else {
        $("#protected_content").removeClass("d-none");
        return true;
...
function setPassword() {
    window.localStorage["suggestions_passwordhash"] = sha1($('#password_input').val());
}